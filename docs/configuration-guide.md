...
**Q：安装包版怎么知道 GPU 加速装好了没？**
首次启动检测到 NVIDIA 显卡会弹「正在后台安装 GPU 加速组件」提示，装完再弹「重启后生效」。日志在 `%LOCALAPPDATA%\com.openmeet.app\logs\gpu_setup.log`，ASR 服务日志在同目录 `asr_service.log`——反馈问题时把这两个文件带上。安装失败会自动回退 CPU 模式，下次启动会再试。

**Q：模型下载失败 / 卡住？**
所有模型走魔搭国内源，正常几分钟能下完。如果公司网络限制了 modelscope.cn，可在能联网的机器上下载后，把缓存目录里的 `models/` 整个拷过去（设置 → 通用 → 缓存目录）。
